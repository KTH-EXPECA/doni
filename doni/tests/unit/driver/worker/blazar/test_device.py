...
from doni.driver.worker.blazar.device import UNKNOWN_DEVICE, BlazarDeviceWorker
from doni.objects.availability_window import AvailabilityWindow
from doni.objects.hardware import Hardware
from doni.tests.unit import utils
from doni.worker import WorkerResult

# For devices in Blazar, the ID column is a UUID
TEST_BLAZAR_RESOURCE_ID = uuidutils.generate_uuid()
TEST_STATE_DETAILS = {
    "blazar_resource_id": TEST_BLAZAR_RESOURCE_ID,
}
...
def _fake_lease(aw_obj):
    lease = BlazarDeviceWorker.to_lease(aw_obj)
...
def blazar_worker(test_config):
    ...
    test_config.config(
        ...
        enabled_worker_types=["blazar.device"],
    )

    worker = BlazarDeviceWorker()
...
def _get_devices_response(hw_list) -> dict:
...
def _stub_blazar_device_exist(path, method, json, hw_list=None, device_details={}):
    """Blazar stub for case where device where matching UUID exists."""
    if method == "get" and path == f"/devices/{TEST_BLAZAR_RESOURCE_ID}":
        return utils.MockResponse(200, {"device": device_details})
    elif method == "get" and path == f"/devices":
...
def test_new_device(
    ...
    blazar_worker: "BlazarDeviceWorker",
    ...
    def _stub_blazar_request(path, method=None, json=None, **kwargs):
        host_response = _stub_blazar_device_new(path, method, json)
        if host_response:
...
def test_existing_device(
    ...
    def _stub_blazar_request(path, method=None, json=None, **kwargs):
        host_response = _stub_blazar_device_exist(path, method, json, hw_list)
        if host_response:
            return host_response
        raise NotImplementedError(f"Unexpected request signature: {method} {path}")
...
def test_no_updates_to_device(
    ...
    def _stub_blazar_request(path, method=None, json=None, **kwargs):
        device_response = _stub_blazar_device_exist(
            path,
            method,
            json,
            hw_list,
...
def test_create_new_lease(
    ...
    def _stub_blazar_request(path, method=None, json=None, **kwargs):
        ...
        host_response = _stub_blazar_device_exist(path, method, json)
        if host_response:
            return host_response
    ...
    # 2 call to devices path, 2 calls to leases
...
def test_update_lease(
    ...
    # 1 call to devices path, 2 calls to leases
    assert blazar_request.call_count == call_count
